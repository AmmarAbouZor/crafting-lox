...
// Error at the previous token. The short name is used here because this
// reporting is used more often than reporting on current token.
...
static void emitReturn() {
  // For function without a return statement;
  emitByte(OP_NIL);
  emitByte(OP_RETURN);
}
...
static void initCompiler(Compiler *compiler, FunctionType type) {
  ...
  // Function assigned to null first to consider the garbage collector.
  ...
  // We declare the local with depth 0 as local for VM internal use only,
  // therefore we give it no name so code users can call it.
...
static void addLocal(Token name) {
  ...
  Local *local = &current->locals[current->localCount++];
  local->name = name;
  // Mark the variable as uninitialized with this value to solve edge cases like
  // `var a = a`;
  local->depth = -1;
  local->isCaptured = false;
}
...
static void function(FunctionType type) {
  ...
  // Note from the book:
  // This beginScope() does not have a corresponding endScope() call. Because we
...
static void parsePrecedence(Precedence precedence) {
  ...
  // variable function will return silently if it can't assign, therefore we
  // need to check here for that and return the error.
  if (canAssign && match(TOKEN_EQUAL)) {
